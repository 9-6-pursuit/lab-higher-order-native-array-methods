...
function getSongsBySaib(songs) {
  return foundSongs = songs.filter(song => song.artist === "Saib")
}
...
function getSongsOverThreeMinutes(songs) {
  return foundSongs = songs.filter(song => song.runtimeInSeconds > 180)
}
...
function getTitleTracks(songs) {
  return foundSongs = songs.filter(song => song.album === song.title)
}
